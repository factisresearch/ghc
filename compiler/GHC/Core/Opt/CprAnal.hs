...
unboxingStrategy env ty dmd
  | Just (dmds, DataConPatContext { dcpc_dc = dc, dcpc_tc_args = tc_args })
      <- wantToUnbox (ae_fam_envs env) has_inlineable_prag ty dmd
  ...
  = Just (dc, map scaledThing (dataConInstArgTys dc tc_args), dmds)
  | otherwise
  = Nothing
  where
