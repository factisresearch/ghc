...
addNotConCt nabla x nalt = do
  (pls_mark_dirty, nabla') <- trvVarInfo go nabla x
  pure $ applyWhen pls_mark_dirty (markDirty x) nabla'
  where
    -- | Update `x`'s 'VarInfo' entry. Fail ('MaybeT') if contradiction,
    -- otherwise return updated entry and `True` if `x` should be marked dirty.
    go :: VarInfo -> MaybeT DsM (Bool, VarInfo)
    go vi@(VI _ pos neg _ rcm) = do
      ...
      pure $ case mb_rcm' of
        ...
        -- have to mark x dirty, by returning True.
        Just rcm' -> (True,  vi'{ vi_rcm = rcm' })
        -- Otherwise, nalt didn't occur in any residual COMPLETE set and we
        -- don't have to mark it dirty. So we return False, which in the case
